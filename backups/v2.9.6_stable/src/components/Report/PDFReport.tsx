...
interface PDFReportProps {
    ...
    allCareers?: any[];
    userName?: string;
    userDetails?: any; // Added to fix prop mismatch
    hasPaid?: boolean;
}
                    ...
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' }}>
                        ...
                        {commGuide && (
                            <View style={{ width: '100%', marginTop: 5, padding: 10, backgroundColor: '#ecfdf5', borderLeftWidth: 3, borderLeftColor: '#10b981', borderRadius: 4 }}>
                                <Text style={{ fontSize: 11, fontWeight: 'bold', color: '#047857', marginBottom: 8 }}>🗣️ My Communication Manual</Text>
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <View style={{ width: '48%' }}>
                                        <Text style={{ fontSize: 9, fontWeight: 'bold', color: '#166534', marginBottom: 4 }}>DO THIS (WITH ME) ✅</Text>
                                        {commGuide.dos.map((d, i) => (
                                            <Text key={i} style={{ fontSize: 9, color: '#14532d', marginBottom: 2 }}>• {d}</Text>
                                        ))}
                                    </View>
                                    <View style={{ width: '48%' }}>
                                        <Text style={{ fontSize: 9, fontWeight: 'bold', color: '#991b1b', marginBottom: 4 }}>AVOID THIS (WITH ME) ❌</Text>
